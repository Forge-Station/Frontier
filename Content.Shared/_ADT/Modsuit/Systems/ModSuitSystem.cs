...
public sealed class ModSuitSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ModSuitComponent, InventoryRelayedEvent<FindInventoryBatteryEvent>>(
            OnFindInventoryBatteryEvent);
    }

    private void GetRelayedVerbs(Entity<ModSuitComponent> modSuit,
    ...
    private void OnGetVerbs(Entity<ModSuitComponent> modSuit, ref GetVerbsEvent<EquipmentVerb> args)
    {
        ...
        if (comp.UserName != null && _id.TryFindIdCard(comp.TempUser.Value, out var id) &&
    ...
    private void OnGetAttachedStripVerbsEvent(Entity<ModAttachedClothingComponent> attached,
    ...
    private void OnModSuitUnequip(Entity<ModSuitComponent> modSuit, ref GotUnequippedEvent args)
    {
        ...
        foreach (var part in parts)
        {
            ...
            _inventorySystem.TryUnequip(args.Equipee,
                part.Value,
                force: true); //TODO: сделать чтобы это работало, а то сейчас писец после гиба
        }
    }
    ...
    private void OnAttachedUnequipAttempt(Entity<ModAttachedClothingComponent> attached,
    ...
    private void OnToggleClothingMessage(Entity<ModSuitComponent> modSuit, ref ModSuitUiMessage args)
    {
        ...
        if (modSuit.Comp.Toggletick + TimeSpan.FromSeconds(0.1) >=
            _timing.CurTime) //маленькие костыли в связи с тем, что ивент проходит 2 раза после нажатия лкм и 3 после нажатия пкм.
            return;
    ...
    private void EquipClothing(EntityUid user, Entity<ModSuitComponent> modSuit, EntityUid clothing, string slot)
    {
        ...
        if (_inventorySystem.TryGetSlotEntity(parent, slot, out var currentClothing))
        {
            ...
            {
                _popupSystem.PopupClient(Loc.GetString("modsuit-remove-first", ("entity", currentClothing)),
                    user,
                    user);
                return;
            }
    ...
    private void OnMapInit(Entity<ModSuitComponent> modSuit, ref MapInitEvent args)
    {
        ...
        _cell.SetDrawEnabled(modSuit.Owner,
            false); //эмэмэмэмэмэм, я не знаю почему, но оно работает только если вначале включить, а потом выключить
        UpdateUserInterface(modSuit.Owner, modSuit.Comp);
    }
    ...
    /// </summary>
    private void OnFindInventoryBatteryEvent(Entity<ModSuitComponent> entity,
        ref InventoryRelayedEvent<FindInventoryBatteryEvent> args)
    {
