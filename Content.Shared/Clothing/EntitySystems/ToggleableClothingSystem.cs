...
public sealed class ToggleableClothingSystem : EntitySystem
{
    ...
    public void ToggleClothing(EntityUid user, EntityUid toggleableUid, EntityUid attachedUid)
    {
        if (!TryComp<ToggleableClothingComponent>(toggleableUid, out var comp))
            return;
        ToggleClothing(user, (toggleableUid, comp), attachedUid);
    }
