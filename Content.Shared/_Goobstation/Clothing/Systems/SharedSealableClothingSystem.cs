...
using Content.Shared.Goobstation.Clothing.Components;
...
namespace Content.Shared.Goobstation.Clothing.Systems;
...
public abstract class SharedSealableClothingSystem : EntitySystem
{
    ...
    private void NextSealProcess(Entity<SealableClothingControlComponent> control)
    {
        ...
        while (true) // ugly but this used to be recursion so if we're doing this that way use iteration instead.
        {
            ...
            var doAfterArgs = new DoAfterArgs(EntityManager, uid, sealableComponent.SealingTime, new SealClothingDoAfterEvent(), uid, target: processingPart, comp.WearerEntity) { NeedHand = false, RequireCanInteract = false, };
